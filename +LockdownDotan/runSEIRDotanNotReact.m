function [x,t] = runSEIRDotanNotReact(xinit, param,...
    TtoQuarantine, options1)
...
tspan=[0 TtoQuarantine];
Tmx=max(tspan);
Tmn=min(tspan);
[t,x] = ode45(@(t,x) mySEIRode(t,x,param),[Tmn Tmx],xinit(:),options1);


%%%%begin quarantine%%%%%
param(1:3)=param(1:3)*0.76;
Tmn=t(end);
Tmx=365; %TtoQuarantine+Tmn;
[t2,x2] = ode45(@(t,x) mySEIRode(t,x,param),[Tmn Tmx],x(end,:)',options1);

x=[x;x2];
t=[t;t2];

% 
% figure;plot(t,H,'LineWidth',2);
% hold all; plot(t,I,'LineWidth',2),hold all

%iext=I';
%tt=t'-14;
%function [x,t]=SEIRodeSolver_YR2(tspan,param,xinit)
...
function dxdt = mySEIRode(t,x,params)
%x vec:
%  X: [S I H R D]
% x(1:4) = S,
