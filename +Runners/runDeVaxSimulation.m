...
R0      = Estimation.calcR0MOH(infected.x1, 1:length(infected.t), 1:length(infected.t));
...
SimInput  = Config.CSimInput("N0",N0,"pH",p_h_l,'pD',pD_l,...  
        'p_susc',p_susc,'p_cautious',0,'R',3, ...
        "corr", 0.5, "tspan", [min(dInds) max(dInds)]);
...
testFunc = @(eff, caution) VaxOffset.testVaxDecrease(eff, caution, SimInput, dInfs, dHosp,...
    initStruct);
...
[vals2, ~, ~, ~, op2]   = lsqnonlin(@(x) VaxOffset.errfun(x, testFunc), vals, b(3:4), b(1:2));
%%
% model "degraded" - after fitting we check the scenario of what-if.
% 1. people vaccinated therefore we get that vals2(1) is the vaccine
% efficiency.
% 2. vals(2) are the number of people not cautious.
% 3. we change the model so that all those who were "cautious" i.e.
% vaccinated, stop complying, and the betaList becomes less of a factor.

% compliance results:
[rates, err, x, t, gofs, runInputs] = VaxOffset.testVaxDecrease(vals2(1), vals2(2),...
    SimInput, dInfs, dHosp, initStruct);



infs = sum(x(:, 5:8), 2);
% now we move to the degraded model: p_transfer is the chance of moving.
% p_nonCompliance is the weakening in caution.
% alternately:
bad_corr_xinit = runInputs.xinit;
% all susc. transfer to not cautious.
p_transfer = 0.1;
p_nonCompliance = 2;
v_efficiency = vals2(1);
SimInput.betaList = [v_efficiency^2 * p_nonCompliance^2 v_efficiency * p_nonCompliance 1];
[~, param_degraded, ~] = SimInput.getODEParams();
...
[x2, t2] = SEIR.SEIRodeSolver_YR(runInputs.tspan, param_degraded, bad_corr_xinit);
...
% build figure
f = figure;
plot(decreaseT, dInfs, "rs", "MarkerFaceColor", [1 0.8 0.8], "MarkerSize", 8,...
